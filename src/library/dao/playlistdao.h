...
class PlaylistDAO : public QObject, public virtual DAO {
  ...
  public:
    ...
    // Delete a playlist
    void deletePlaylist(PlaylistId playlistId);
    ...
    // Rename a playlist
    void renamePlaylist(PlaylistId playlistId, const QString& newName);
    // Lock or unlock a playlist
    bool setPlaylistLocked(PlaylistId playlistId, const bool locked);
    ...
    bool isPlaylistLocked(PlaylistId playlistId) const;
    // Check if a playlist exists
    bool playlistExists(PlaylistId playlistId) const;
    // Append a list of tracks to a playlist
    bool appendTracksToPlaylist(const QList<TrackId>& trackIds, PlaylistId playlistId);
    // Append a track to a playlist
    bool appendTrackToPlaylist(TrackId trackId, PlaylistId playlistId);
    ...
    QString getPlaylistName(PlaylistId playlistId) const;
    ...
    QList<TrackId> getTrackIds(PlaylistId playlistId) const;
    QList<TrackId> getTrackIdsInPlaylistOrder(PlaylistId playlistId) const;
    QList<TrackId> getAutoDJTrackIds() const;
    // Returns true if the playlist with playlistId is hidden
    bool isHidden(PlaylistId playlistId) const;
    // Returns the HiddenType of playlistId
    HiddenType getHiddenType(PlaylistId playlistId) const;
    // Returns the maximum position of the given playlist
    int getMaxPosition(PlaylistId playlistId) const;
    ...
    void removeHiddenTracks(PlaylistId playlistId);
    // Remove a track from a playlist
    void removeTrackFromPlaylist(PlaylistId playlistId, int position);
    void removeTracksFromPlaylist(PlaylistId playlistId, const QList<int>& positions);
    void removeTracksFromPlaylistById(PlaylistId playlistId, TrackId trackId);
    // Insert a track into a specific position in a playlist
    bool insertTrackIntoPlaylist(TrackId trackId, PlaylistId playlistId, int position);
    // Inserts a list of tracks into playlist
    int insertTracksIntoPlaylist(const QList<TrackId>& trackIds, PlaylistId playlistId, int position);
    // Remove all tracks from the Auto-DJ Queue
    void clearAutoDJQueue();
    // Add a playlist to the Auto-DJ Queue
    void addPlaylistToAutoDJQueue(PlaylistId playlistId, AutoDJSendLoc loc);
    ...
    int getPreviousPlaylist(PlaylistId currentPlaylistId, HiddenType hidden) const;
    // Get the following playlist of currentPlaylistId with the HiddenType
    // hidden. Returns -1 if no such playlist exists.
    int getNextPlaylist(PlaylistId currentPlaylistId, HiddenType hidden) const;
    // Append all the tracks in the source playlist to the target playlist.
    bool copyPlaylistTracks(PlaylistId sourcePlaylistID, PlaylistId targetPlaylistID);
    // Returns the number of tracks in the given playlist.
    int tracksInPlaylist(PlaylistId playlistId) const;
    ...
    void orderTracksByCurrPos(PlaylistId playlistId, QList<std::pair<TrackId, int>>& newOrder);
    // moved Track to a new position
    void moveTrack(PlaylistId playlistId,
            const int oldPosition, const int newPosition);
    // shuffles all tracks in the position List
    void shuffleTracks(PlaylistId playlistId, const QList<int>& positions, const QHash<int,TrackId>& allIds);
    bool isTrackInPlaylist(TrackId trackId, PlaylistId playlistId) const;
  ...
  private:
    bool removeTracksFromPlaylist(PlaylistId playlistId, int startIndex);
    void removeTracksFromPlaylistInner(PlaylistId playlistId, int position);
    void removeTracksFromPlaylistByIdInner(PlaylistId playlistId, TrackId trackId);
