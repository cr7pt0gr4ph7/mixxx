...
#include "moc_httpserverplaylistmodel.cpp"
#include "track/track.h"

#define HTTP_SERVER_TABLE "httpserver"
...
HttpServerPlaylistModel::HttpServerPlaylistModel(QObject* pParent, TrackCollectionManager* pTrackCollectionManager, HttpServerConnection* pConnection)
        : BaseSqlTableModel(pParent, pTrackCollectionManager, "mixxx.db.model.httpserver_playlist"),
          m_pConnection(pConnection),
          m_playlistId(kInvalidPlaylistId) {
    m_tempTableName = HTTP_SERVER_TABLE + QString::number(sTableNumber.fetchAndAddAcquire(1));
}

HttpServerPlaylistModel::~HttpServerPlaylistModel() {
    dropTempTable();
}

void HttpServerPlaylistModel::dropTempTable() {
    if (m_playlistId >= 0) {
        // Clear old playlist
        m_playlistId = kInvalidPlaylistId;
...
void HttpServerPlaylistModel::selectPlaylist(int playlistId) {
    // qDebug() << "HttpServerPlaylistModel::selectPlaylist" << this << playlistId;
    ...
    if (playlistId >= 0) {
        ...
        if (!list.isEmpty()) {
            ...
            foreach (struct HttpServerConnection::PlaylistEntry entry, list) {
...
TrackModel::Capabilities HttpServerPlaylistModel::getCapabilities() const {
...
Qt::ItemFlags HttpServerPlaylistModel::flags(const QModelIndex& index) const {
    return readOnlyFlags(index);
}

TrackId HttpServerPlaylistModel::doGetTrackId(const TrackPointer& pTrack) const {
    if (pTrack) {
...
QVariant HttpServerPlaylistModel::getFieldVariant(const QModelIndex& index,
        const QString& fieldName) const {
    return index.sibling(index.row(), fieldIndex(fieldName)).data();
}

QString HttpServerPlaylistModel::getFieldString(const QModelIndex& index,
        const QString& fieldName) const {
    return getFieldVariant(index, fieldName).toString();
}

TrackPointer HttpServerPlaylistModel::getTrack(const QModelIndex& index) const {
    ...
    // saved with the metadata from HttpServer. If it was already in the library
...
TrackId HttpServerPlaylistModel::getTrackId(const QModelIndex& index) const {
...
QUrl HttpServerPlaylistModel::getTrackUrl(const QModelIndex& index) const {
    if (!index.isValid()) {
        return {};
    }
...
QString HttpServerPlaylistModel::getTrackLocation(const QModelIndex& index) const {
...
bool HttpServerPlaylistModel::isColumnInternal(int column) {
