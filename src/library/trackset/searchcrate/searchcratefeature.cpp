#include "library/trackset/crate/SearchCrateFeature.h"
...
#include "library/trackset/crate/SearchCrateFeaturehelper.h"
...
void SearchCrateFeature::initActions() {
    ...
    connect(m_pAnalyzeCrateAction.get(),
            &QAction::triggered,
            this,
            &SearchCrateFeature::slotAnalyzeCrate);
    ...
    connect(m_pExportAllCratesAction.get(),
            &QAction::triggered,
            this,
            &SearchCrateFeature::exportAllCrates);
...
void SearchCrateFeature::connectTrackCollection() {
    connect(m_pTrackCollection, // created new, duplicated or imported playlist to new crate
            ...
            this,
            &SearchCrateFeature::slotCrateTableChanged);
    ...
    connect(m_pTrackCollection, // crate tracks hidden, unhidden or purged
            ...
            this,
            &SearchCrateFeature::slotCrateContentChanged);
    connect(m_pTrackCollection,
            &TrackCollection::crateSummaryChanged,
            this,
            &SearchCrateFeature::slotUpdateCrateLabels);
}
...
TreeItem* SearchCrateFeature::getOrCreateTreeItemForCrateId(
        CrateId crateId) {
    if (!crateId.isValid()) {
...
std::unique_ptr<TreeItem> SearchCrateFeature::newTreeItemForCrateSummary(
        const CrateSummary& crateSummary) {
...
void SearchCrateFeature::updateTreeItemForCrateSummary(
...
bool SearchCrateFeature::dropAccept(const QList<QUrl>& urls, QObject* pSource) {
    Q_UNUSED(pSource);
...
bool SearchCrateFeature::dragMoveAccept(const QList<QUrl>& urls) {
...
bool SearchCrateFeature::activateCrate(CrateId crateId) {
    qDebug() << "SearchCrateFeature::activateCrate()" << crateId;
    VERIFY_OR_DEBUG_ASSERT(crateId.isValid()) {
        return false;
    }
...
bool SearchCrateFeature::readLastRightClickedCrate(Crate* pCrate) const {
    CrateId crateId(crateIdFromIndex(m_lastRightClickedIndex));
    VERIFY_OR_DEBUG_ASSERT(crateId.isValid()) {
...
void SearchCrateFeature::slotCreateCrate() {
    Crate crate;
...
void SearchCrateFeature::slotCreateSubCrate() {
    CrateId crateId(crateIdFromIndex(m_lastRightClickedIndex));
    if (!crateId.isValid()) {
        return;
    }
...
void SearchCrateFeature::createNewCrate(CrateId parentId, bool selectAfterCreation) {
...
bool SearchCrateFeature::moveToParent(CrateId destinationId, const QList<CrateId>& cratesToMove) {
...
bool SearchCrateFeature::moveToParent(CrateId destinationId, CrateId crateToMoveId, bool selectAfterMove) {
    // Note: An "invalid"/NULL destination is not actually invalid
    //       for this function, but instead represents the root folder.
    Crate crate;
...
void SearchCrateFeature::slotDeleteCrate() {
    Crate crate;
    if (readLastRightClickedCrate(&crate)) {
        if (crate.isLocked()) {
...
void SearchCrateFeature::slotRenameCrate() {
    Crate crate;
    if (readLastRightClickedCrate(&crate)) {
...
void SearchCrateFeature::slotDuplicateCrate() {
    Crate crate;
    if (readLastRightClickedCrate(&crate)) {
        CrateId newCrateId =
                SearchCrateFeatureHelper(m_pTrackCollection, m_pConfig)
...
void SearchCrateFeature::slotToggleCrateLock() {
    Crate crate;
    if (readLastRightClickedCrate(&crate)) {
...
QModelIndex SearchCrateFeature::rebuildChildModel(CrateId selectedCrateId) {
    qDebug() << "SearchCrateFeature::rebuildChildModel()" << selectedCrateId;
...
void SearchCrateFeature::updateChildModel(const QSet<CrateId>& updatedCrateIds) {
...
CrateId SearchCrateFeature::crateIdFromIndex(const QModelIndex& index) const {
    if (!index.isValid()) {
        return CrateId();
    }
...
QModelIndex SearchCrateFeature::indexFromCrateId(CrateId crateId) const {
    VERIFY_OR_DEBUG_ASSERT(crateId.isValid()) {
        return QModelIndex();
    }
    if (crateId.isValid()) {
...
void SearchCrateFeature::slotImportPlaylistFile(const QString& playlistFile, CrateId crateId) {
    Crate crate;
    if (!m_pTrackCollection->crates().readCrateById(crateId, &crate)) {
        return;
    }
...
void SearchCrateFeature::slotCreateImportCrate() {
    slotCreateImportPlaylist(CrateId());
}

void SearchCrateFeature::slotCreateImportSubCrate() {
    slotCreateImportPlaylist(crateIdFromIndex(m_lastRightClickedIndex));
}

void SearchCrateFeature::slotCreateImportPlaylist(CrateId parentId) {
    // Get file to read
...
void SearchCrateFeature::slotAnalyzeCrate() {
...
void SearchCrateFeature::storePrevSiblingCrateId(CrateId crateId) {
...
void SearchCrateFeature::slotCrateTableChanged(CrateId crateId) {
    Q_UNUSED(crateId);
...
void SearchCrateFeature::slotCrateContentChanged(CrateId crateId) {
    QSet<CrateId> updatedCrateIds;
    updatedCrateIds.insert(crateId);
    updateChildModel(updatedCrateIds);
}

void SearchCrateFeature::slotUpdateCrateLabels(const QSet<CrateId>& updatedCrateIds) {
    updateChildModel(updatedCrateIds);
}
