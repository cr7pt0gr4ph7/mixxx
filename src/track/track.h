...
class Track : public QObject {
  ...
  public:
    ...
    //
    // This was originally only the four digit (gregorian) calendar year
    // of the release date, but still allowed to store any user string.
    //
    // Now it is alternatively used as the recording date/time in the
    // ISO 8601 yyyy-MM-ddTHH:mm:ss format truncated at any point,
    // following the TDRC ID3v2.4 frame, or when the latter does
    // not exist, TYER + TDAT.
